<x-app-layout>
    ...
    <div class="py-8 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if(session('answer'))
            <div class="mb-6 space-y-3">
                <div class="bg-indigo-50 dark:bg-indigo-900 rounded-lg p-4">
                    <p class="text-xs text-indigo-400 mb-1">Tú preguntaste:</p>
        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 mb-6">
            <form method="POST" action="{{ route('client.chat.ask') }}">
                @csrf
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    ¿En qué puedo ayudarte?
                </label>
                <textarea name="prompt" rows="3"
                          placeholder="Ej: ¿Cuántos pedidos tengo? ¿Cuáles son mis últimos pedidos?"
